...
router.post('/', authenticate, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!questions || questions.length === 0) {
      return res
        .status(400)
        .json({ message: 'No questions found for the specified difficulty.' });
    }
    ...
    if (newGame.questions.length !== newGame.questionsCount) {
      console.error(
        'Mismatch between questions and questionsCount:',
        newGame.questions.length,
        newGame.questionsCount
      );
      return res
        .status(500)
        .json({ message: 'Mismatch between questions and questionsCount.' });
    }
...
router.get('/latest', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const gamesWithUserNames = await Promise.all(
      latestGames.map(async (game: IGame) => {
        const db = admin.firestore();
